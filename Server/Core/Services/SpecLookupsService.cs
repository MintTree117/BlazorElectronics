...
public sealed class SpecLookupsService : _CachedApiService, ISpecLookupsService
{
    readonly ISpecRepository _repository;
    readonly ICategoryService _categoryService;
    LookupSpecsDto? _cachedSpecData;

    public SpecLookupsService( ILogger<SpecLookupsService> logger, ISpecRepository repository, ICategoryService categoryService )
    ...
    protected override void UpdateCache()
    {
        try
        {
            ...
            ServiceReply<List<int>?> ids = Task
                .Run( () => _categoryService.GetPrimaryIds() ).Result;

            if ( !ids.Success || ids.Payload is null )
                return;

            MapResponse( models, ids.Payload );
        }
    ...
    public async Task<ServiceReply<LookupSpecsDto?>> Get()
    {
        ...
        try
        {
            ServiceReply<List<int>?> categoryReply = await _categoryService.GetPrimaryIds();
            ...
            LookupSpecsDto? response = MapResponse( model, categoryReply.Payload );
